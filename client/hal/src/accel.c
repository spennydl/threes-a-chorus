...
Accel_open(void)
{
    ...
    if (I2C_selectDevice(&i2c, 0x18) < 0) {
        perror("i2c select device");
        return -1;
    }

    uint8_t cfg = (1 << 5) | 0x7;
    if (I2C_write(&i2c, 0x20, &cfg, 1) < 0) {
        perror("config");
        I2C_closeBus(&i2c);
        return -1;
    }
...
Accel_read(Accel_Sample* out)
{
    ...
    out->x = (((buf[1] << 8) | buf[0]) >> 4);
    /* Then subtract 2^11 if the high bit is set. */
    out->x -= ((1 << 11) & out->x) << 1;

    /* And repeat for the other axes. */
    out->y = ((buf[3] << 8) | buf[2]) >> 4;
    out->y -= ((1 << 11) & out->y) << 1;

    out->z = ((buf[5] << 8) | buf[4]) >> 4;
    out->z -= ((1 << 11) & out->z) << 1;
    /* The Z register will always feel ~1G from normal gravity when it is on the
     * table. We make a crude correction for it. */
    // out->z -= 1024;
